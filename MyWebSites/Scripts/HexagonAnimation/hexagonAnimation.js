...
let currentPage = 1;
const hexagonsPerPage = 3000; // Number of hexagons per page
...
function createHexagons() {
  ...
  for (let row = 0; row < numRows; row++) {
    for (let col = 0; col < numCols; col++) {
      if (hexagonsCreated >= hexagonsPerPage) {
        return; // Stop if we've reached the limit of hexagons for this page
      }

      const hexagon = document.createElement('div');
      hexagon.classList.add('hexagon'); // Add hexagon class to apply animation
      ...
      // Apply the staggered animation to each hexagon
      const delay = hexagonsCreated * 0.05; // Stagger delay by 0.05s for each hexagon
      hexagon.style.animationDelay = `${delay}s`; // Add delay to each hexagon's animation
...
window.addEventListener('scroll', () => {
  ...
  if (scrollTop + clientHeight >= scrollHeight - 100) { // Near bottom of page
    currentPage++;
    createHexagons(); // Load more hexagons when scrolled to the bottom
  }
});
